...
public class OrdersEndpointTests {
	 ...
	 */
	public void UserStory1and3_4people_4starters_4mains_4drinks(){
		...
		def response = WS.sendRequest(findTestObject("API/APIOrder1"))
		...
		orderCalculated = orderCalculated.valueOf((starter * 4) + (main * 4));

		//Calculates the 10% Service charge on ONLY the food
		orderCalculated = (orderCalculated * 1.10) + (drink *4.00)
		...
		//This compares the values from the JSON to our own calculation to ensure th test passes
		...
		//@Test
		/* 3a. 1 person from the group cancels their order
		 *	  When the order is sent to the endpoint the total is recalculated correctly
		 */

		response = WS.sendRequest(findTestObject("API/APIOrder3a"))
		...
		//Calculates the Starters and mains separately from the drinks and service
		orderCalculated = orderCalculated.valueOf((starter * 3) + (main * 3));

		//Calculates the 10% Service charge on ONLY the food
		orderCalculated = (orderCalculated * 1.10) + (drink *3.00)
	 ...
	 */
	public void UserStory2_2people_1starter_2mains(){
		...
		def response = WS.sendRequest(findTestObject("API/APIOrder2"))
